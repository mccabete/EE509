...
inits_tess<-list()
inits_tess[[1]] <- list(mu = mean(data$N))
inits_tess[[2]] <- list(mu = (mean(data$N)-(mean(data$N)*(1/4))))
inits_tess[[3]] <- list(mu = mean(data$N)+(mean(data$N)*(1/4)))
...
jags.out   <- coda.samples (model = j.model,
                            variable.names = c("mu"),
                                n.iter = 1000)
plot(jags.out) #Not realy yet, haven't subset the burn in. 
```
