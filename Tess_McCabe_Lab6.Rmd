...
cumuplot(var.out,probs=c(0.025,0.25,0.5,0.75,0.975)) # Eyeballing this, looks like I'm pretty safe after 1000 iteractions. 
...
multivariate_regression <- "model{
  ...
  for(i in 1:n){
	  mu[i] <- beta[1] + beta[2]*x[i] + beta[3]*x[i] +beta[4]*x[i]  	## process model
	  y[i]  ~ dnorm(mu[i],prec)		## data model
  }
}
"
...
cumuplot(var.out,probs=c(0.025,0.25,0.5,0.75,0.975)) # Eyeballing this time , still looks like I'm pretty safe after 1000 iteractions. 
var.burn <-var.temp[1000:dim(var.temp)[1],]

# Retrying effective sample size
effectiveSize(var.burn) # looks good
...
summary(var.burn)
```
The model was decent for recovering the intercept, but not as good at recovering the slope. The mean slope was consistantly off, althought the covariation is closer. That the intercept was more easily modeled than the slopes makes sence, as it is dependant on less infomration than a slope. All the "true" values were within the 25% to 75% quartiles. 
...
## credible and prediction intervals
nsamp <- length(xpred)
samp <- sample.int(nrow(var.temp.5),nsamp)
xpred <- order(x1)  					## sequence of x values we're going to
npred <- length(xpred)				##      make predictions for
ypred <- matrix(0.0,nrow=nsamp,ncol=npred)	## storage for predictive interval
ycred <- matrix(0.0,nrow=nsamp,ncol=npred)	## storage for credible interval
...
lines(xpred,ci[1,],col=3,lty=2, type = "p")	## lower CI
lines(xpred,ci[2,],col=3,lwd=3, type = "p")	## median
lines(xpred,ci[3,],col=3,lty=2, type = "p")	## upper CI
lines(xpred,pi[1,],col=4,lty=2, type = "p")	## lower PI
lines(xpred,pi[2,],col=4,lty=2, type = "p")	## upper PI
```
...
nsamp <- length(x1)
samp <- sample.int(nrow(var.temp.5),nsamp)
xpred <- x1					## sequence of x values we're going to
npred <- length(xpred)				##      make predictions for
ypred <- matrix(0.0,nrow=nsamp,ncol=npred)	## storage for predictive interval
ycred <- matrix(0.0,nrow=nsamp,ncol=npred)	## storage for credible interval
