...
Our JAGS model for regression is very similar to our model from Lab 5. However, rather than having a single, fixed mu (expected value of y), we now index mu to vary by observation. We then add a linear process model to predict how mu changes as a function of x, and then add a multivariate normal prior on the regression parameters (beta).
...
The last thing we need to do to prep for the MCMC loop is to specify the **initial conditions**. As with Lab 5, we'll want to put these in a list. Unlike Lab 5, we're specifying multiple initial conditions and creating a list of lists so that we can give different initial conditions to each chain.
...
As in Lab 5, we'll pass the model, data, and inits to the `jags.model` function in order to compile and initialize the model
...
As we did in Lab 5, we need to evaluate the output of the MCMC and will use the “coda” library to simplify these graphs and calculations. 
...
### Lab Report Task 4:  Informative Priors (Extra Credit)
